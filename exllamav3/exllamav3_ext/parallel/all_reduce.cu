...
// P2P-optimized all_reduce kernel using direct GPU-to-GPU communication
#define MAX_NUM_THREADS_P2P 1024
#define BATCH_STAGE_P2P 4

__global__ __launch_bounds__(MAX_NUM_THREADS_P2P)
void pg_all_reduce_full_p2p_kernel
(
    PGContext* __restrict__ ctx,
    uint32_t device_mask,
    int this_device,
...
)
{
    ...
    auto wait_min_stage = [&] (uint32_t* stage_ptr, int min_stage, uint64_t deadline)
    {
        if (t == 0)
        {
            ...
            while ((int) ldg_acquire_sys_u32(stage_ptr) < min_stage)
            {
                __nanosleep(sleep);
                if (sleep < SYNC_MAX_SLEEP) sleep <<= 1;
                else *abort_flag = check_timeout(ctx, deadline, "all_reduce_p2p");
                if (*abort_flag) break;
            }
        }
        __syncthreads();
    ...
    // Send to next rank, receive from previous rank using P2P
    int this_rank = __popc(device_mask & ((1 << this_device) - 1));
    ...
    for (int iter = 0; iter < (num_ranks - 1) * 2; ++iter)
    {
        ...
        if (dir == 0)
        {
            while (stage_recv < stage_end)
            {
                ...
                if (stage_recv < stage_ready_val)
                {
                    while (stage_recv < stage_ready_val)
                    {
                        sleep = SYNC_MIN_SLEEP;

                        // First num_ranks - 1 iterations: accumulate using P2P
                        if (iter < num_ranks - 1)
                        {
                            float4* src = (float4*) shbuf_stage_ptr(this_rank, stage_recv);
                            float4* dst = (float4*) data_stage_ptr(recv_seg, stage_recv);
                        ...
                        // Last num_ranks - 1 iterations: copy using P2P
                        else
                        {
                            uint4* src = (uint4*) shbuf_stage_ptr(this_rank, stage_recv);
                            uint4* dst = (uint4*) data_stage_ptr(recv_seg, stage_recv);
                ...
                else
                {
                    __nanosleep(sleep);
                    if (sleep < SYNC_MAX_SLEEP) sleep <<= 1;
                    else *abort_flag = check_timeout(ctx, deadline, "all_reduce_p2p (1)");
                    if (*abort_flag) break;
                }
            }
        }

        // Send using P2P direct memory access
        if (dir == 1)
        {
            while (stage_send < stage_end)
            {
                ...
                if (ready_send)
                {
                    for (int i = 0; i < BATCH_STAGE_P2P && stage_send < stage_end; ++i)
                    {
                        sleep = SYNC_MIN_SLEEP;
                        uint4* src = (uint4*) data_stage_ptr(send_seg, stage_send);
                        uint4* dst = (uint4*) shbuf_stage_ptr(next_device, stage_send);
                        if (src + t < (uint4*) data_end) dst[t] = src[t];
                ...
                else
                {
                    __nanosleep(sleep);
                    if (sleep < SYNC_MAX_SLEEP) sleep <<= 1;
                    else *abort_flag = check_timeout(ctx, deadline, "all_reduce_p2p (2)");
                    if (*abort_flag) break;
                }
            }
...
)
{
    ...
    // Convert uintptr_t devices to int devices for P2P operations
    std::vector<int> peer_devices;
    for (auto dev : devices) {
        peer_devices.push_back(static_cast<int>(dev));
    }

    int threads = (int) CEIL_DIVIDE(CEIL_DIVIDE(data_size / 16ll, num_ranks), 32ll) * 32ll;
    threads = MIN(threads, MAX_NUM_THREADS_P2P);
