...
#define NUM_THREADS_P2P_GATHER 1024
#define STAGE_SIZE_P2P (NUM_THREADS_P2P_GATHER * 16)
...
__global__ __launch_bounds__(NUM_THREADS_P2P_GATHER)
void pg_gather_full_p2p_kernel
(
    PGContext* __restrict__ ctx,
    uint32_t device_mask,
    int this_device,
...
)
{
    ...
    auto wait_min_stage = [&] (uint32_t* stage_ptr, int min_stage, uint64_t deadline)
    {
        if (t == 0)
        {
            ...
            while ((int) ldg_acquire_sys_u32(stage_ptr) < min_stage)
            {
                __nanosleep(sleep);
                if (sleep < SYNC_MAX_SLEEP) sleep <<= 1;
                else *abort_flag = check_timeout(ctx, deadline, "gather_p2p (0)");
                if (*abort_flag) break;
            }
        }
        __syncthreads();
    ...
    if (is_consumer)
    {
        // Gather data from all peer devices using P2P
        size_t stride = (size_t) all_offsets[MAX_DEVICES];

        for (int src_rank = 0; src_rank < num_ranks; ++src_rank)
        {
            int src_device = peer_devices[src_rank];
            size_t src_ldim = (size_t) (all_offsets[src_device + 1] - all_offsets[src_device]);
            size_t bytes_to_recv = src_ldim * (size_t) batch;
            size_t dst_offset = (size_t) (all_offsets[src_device]);
            int num_stages = (int) CEIL_DIVIDE(bytes_to_recv, stage_size);
            ...
            while (stage_recv < num_stages)
            {
                ...
                else
                {
                    __nanosleep(sleep);
                    if (sleep < SYNC_MAX_SLEEP) sleep <<= 1;
                    else *abort_flag = check_timeout(ctx, deadline, "gather_p2p (1)");
                    if (*abort_flag) break;
                }
            }
        }
    ...
    else
    {
        ...
        while (stage_send < num_stages)
        {
            bool ready_send = stage_send < num_stages;
            if (ready_send && !no_overflow)
                if (!stage_ready(ctx->gather_stage_consumed + this_rank, stage_send - num_buf_stages + 1 + 2))
                   ready_send = false;

            if (ready_send)
            {
                for (int i = 0; i < 2 && stage_send < num_stages; ++i)
                {
                    sleep = SYNC_MIN_SLEEP;
                    uint4* src = (uint4*) data_stage_ptr(stage_send);
                    uint4* dst = (uint4*) shbuf_stage_ptr(this_rank, stage_send);
            ...
            else
            {
                __nanosleep(sleep);
                if (sleep < SYNC_MAX_SLEEP) sleep <<= 1;
                else *abort_flag = check_timeout(ctx, deadline, "gather_p2p (2)");
                if (*abort_flag) break;
            }
        }
...
)
{
    ...
    dim3 block_grid(num_blocks);
    dim3 block_dim(NUM_THREADS_P2P_GATHER);
